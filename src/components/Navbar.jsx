...
const Navbar = () => {
  ...
  return (
    <motion.nav
      ...
      className={`sticky top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled || isOpen ? 'bg-portfolio-background/80 backdrop-blur-md shadow-lg' : 'bg-transparent'}`}
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <Link to="/" onClick={() => window.scrollTo(0,0)} className="flex items-center space-x-2 text-2xl font-bold text-foreground hover:text-portfolio-primary transition-colors">
          ...
          <div className="hidden md:flex items-center space-x-4 lg:space-x-6">
            {navItems.map((item) => (
              item.isPage ? (
                ...
                >
                  {item.name}
                  <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-portfolio-primary transform ${(location.pathname === item.href && item.href === '/') || (item.href !=='/' && location.pathname.startsWith(item.href)) ? 'scale-x-100' : 'scale-x-0'} group-hover:scale-x-100 transition-transform duration-300 ease-out`}></span>
                </Link>
              ) : (
                <a
                  key={item.name}
            ...
            <Button variant="ghost" size="icon" onClick={toggleTheme} className="text-portfolio-accent hover:text-foreground">
